    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200/80 p-4 rounded-2xl flex items-center gap-4 shadow-xs">
          ...
          <div>
            ...
            <h3 className="text-lg font-extrabold text-[#7a35ff] mt-0.5">{formatCurrency(totalRevenue)}</h3>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200/80 p-4 rounded-2xl flex items-center gap-4 shadow-xs">
          ...
          <div>
            ...
            <h3 className="text-lg font-extrabold text-slate-900 mt-0.5">{totalCount} Transaksi</h3>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200/80 p-4 rounded-2xl flex items-center gap-4 shadow-xs">
          ...
          <div>
            ...
            <h3 className="text-lg font-extrabold text-slate-900 mt-0.5">{formatCurrency(avgOrderValue)}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-white px-4 py-2.5 rounded-xl border border-slate-200 shadow-2xs">
          <Sun className="w-4 h-4 text-[#7a35ff]" />
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider border-b border-slate-200 font-bold text-[11px]">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="p-4 font-mono font-bold text-[#7a35ff]">{order.orderNumber}</td>
                    ...
                    <td className="p-4 font-extrabold text-slate-900">{formatCurrency(order.totalAmount)}</td>
                    <td className="p-4 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 bg-[#f3eeff] hover:bg-[#e6d6ff] text-[#7a35ff] font-semibold rounded-lg flex items-center gap-1.5 ml-auto text-xs transition-colors"
                      >
          ...
          <div className="text-center py-16 text-slate-500 space-y-2">
            ...
            <p className="text-xs text-slate-400">Transaksi kasir hari ini akan muncul di sini secara otomatis.</p>
          </div>
        )}
      </div>
        ...
        {selectedOrder && (
          <div className="space-y-4 text-xs font-mono text-slate-700">
            <div className="text-center pb-3 border-b border-dashed border-slate-200">
              <h4 className="font-sans text-base font-bold text-slate-900">POS ZALDE STORE</h4>
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {selectedOrder.items?.map((item) => (
                <div key={item.id} className="flex justify-between">
                  <div>
                    <p className="font-sans font-semibold text-slate-900">{item.product?.name || 'Produk'}</p>
            ...
            <div className="pt-3 border-t border-dashed border-slate-200 space-y-1 text-slate-600">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-[#7a35ff]">{formatCurrency(selectedOrder.totalAmount)}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span>Kembalian:</span>
                <span className="font-bold text-slate-900">{formatCurrency(selectedOrder.changeAmount)}</span>
              </div>
            </div>
            ...
            <div className="pt-3">
              <button
                ...
                className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-sans text-xs font-semibold rounded-xl flex items-center justify-center gap-2 transition-colors"
              >
